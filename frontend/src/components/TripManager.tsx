            ...
            {currentTripId && (
              <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'center' }}>
                <button
                  ...
                  disabled={isSaving}
                  style={{
                    background: completedCount > 0 ? 'linear-gradient(135deg, var(--accent), #00c97a)' : 'rgba(255,255,255,0.08)',
                    color: completedCount > 0 ? '#000' : 'var(--text-dim)',
                    border: 'none',
                    borderRadius: '14px',
                    padding: '14px 40px',
                    fontWeight: 800,
                    ...
                    alignItems: 'center',
                    gap: '8px',
                    boxShadow: completedCount > 0 ? '0 4px 20px rgba(16,208,122,0.3)' : 'none',
                ...
                >
                  {isSaving ? (
                    <><Loader2 size={18} className="tm-spinner" /> Saving…</>
                  ) : (
                    <>{completedCount > 0 ? '✅' : '☐'} Save Packing List ({completedCount}/{checklist.length} packed)</>
                  )}
                </button>
              </div>
            )}
